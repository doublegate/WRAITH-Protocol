...
{}",
            ...
            match unsafe { self.drop_runner(runner_path) } {
                Ok(_) => {
                    ...
                    let res = ClrHost::execute_assembly(runner_path, "Wraith.Runner", "Run", &arg);
                    ...
                    match res {
                        Ok(exit_code) => {
                            if let Some(out_bytes) = output {
                                ...
                                let s = format!(
                                    "Executed via CLR. Exit code: {} (Output missing)",
                                    exit_code
                                );
    ...
    unsafe fn drop_runner(&self, path: &str) -> Result<(), ()> {
        ...
        if RUNNER_DLL[0] != 0x4D || RUNNER_DLL[1] != 0x5A {
            // MZ
            return Err(());
        }
