...
impl ProtocolHandler {
    pub fn new(
        ...
        Self { db, session_manager, static_key, event_tx }
    }
            ...
            if plaintext.len() > 28 {
                ...
                if let Ok(beacon) = serde_json::from_slice::<BeaconData>(inner_payload) {
                    ...
                    let pending = if implant_uuid != uuid::Uuid::nil() {
                        self.db.get_pending_commands(implant_uuid).await.unwrap_or_default()
                    } else {
                        vec![]
                    };

                    let tasks = pending.into_iter().map(|c| BeaconTask {
                        id: c.id.to_string(),
                        type_: c.command_type,
                        payload: String::from_utf8_lossy(c.payload.as_deref().unwrap_or(&[])).to_string(),
                    }).collect();
