    ...
    for entry in std::fs::read_dir(playbooks_dir)? {
        ...
        if path.is_file() {
            if let Some(ext) = path.extension() {
                ...
                // Parse metadata
                let name = json_content["name"].as_str().unwrap_or("Unknown").to_string();
                let description = json_content["description"].as_str().unwrap_or("").to_string();
                ...
                match db.create_playbook(&name, &description, json_content).await {
                    ...
                    Err(e) => warn!("Failed to load playbook {} (might already exist): {}", name, e),
                }
            }
        }
    }
