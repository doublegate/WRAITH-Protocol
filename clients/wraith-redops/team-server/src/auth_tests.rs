...
fn test_auth_interceptor_whitelists_authenticate() {
    ...
    req.extensions_mut().insert(RpcPath("/wraith.redops.OperatorService/Authenticate".to_string()));
    
    let result = wrap_auth_interceptor(req);
    
    assert!(result.is_ok(), "Authenticate should be whitelisted even without header");
}
...
fn test_auth_interceptor_accepts_valid_token() {
    ...
    assert!(result.is_ok());
    let claims = result.unwrap().extensions().get::<crate::utils::Claims>().cloned().unwrap();
