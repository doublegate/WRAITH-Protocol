...
mod tests {
    ...
    async fn test_operator_service_comprehensive() {
        ...
        sqlx::query(
            "INSERT INTO operators (id, username, public_key, role) VALUES ($1, $2, $3, $4)",
        )
        .bind(op_id)
        .bind("testuser")
        .bind(vec![0u8; 32])
        ...
        // 1. Campaign Tests
        let camp_resp = service
            .create_campaign(auth_req(
        ...
        service
            ...
            .unwrap();
        service
            .update_campaign(Request::new(UpdateCampaignRequest {
                id: camp_id.clone(),
                name: "New".to_string(),
        ...
        sqlx::query(
            "INSERT INTO implants (id, campaign_id, hostname, status) VALUES ($1, $2, $3, $4)",
        )
        .bind(imp_id)
        ...
        service
            .get_implant(Request::new(GetImplantRequest {
                id: imp_id.to_string(),
            }))
            .await
            .unwrap();
        service
            .list_implants(Request::new(ListImplantsRequest {
        ...
        service
            ...
            .unwrap();
        service
            .cancel_command(Request::new(CancelCommandRequest {
        ...
        let list_resp = service
            .create_listener(Request::new(CreateListenerRequest {
                name: "L1".to_string(),
        ...
        service
            .list_listeners(Request::new(ListListenersRequest {}))
            .await
            .unwrap();
        // start/stop might fail if port is taken, but we test the RPC logic
        let _ = service
            .start_listener(Request::new(ListenerActionRequest {
                id: list_id.clone(),
            }))
            .await;
        let _ = service
            .stop_listener(Request::new(ListenerActionRequest {
                id: list_id.clone(),
            }))
            .await;
        ...
        let chain_resp = service
            .create_attack_chain(Request::new(CreateAttackChainRequest {
        ...
        service
            .get_attack_chain(Request::new(GetAttackChainRequest {
                id: chain_id.clone(),
            }))
            .await
            .unwrap();
        service
            .list_attack_chains(Request::new(ListAttackChainsRequest {
                page_size: 10,
                page_token: "".to_string(),
            }))
            .await
            .unwrap();
        service
            .execute_attack_chain(Request::new(ExecuteAttackChainRequest {
        ...
        service
            .list_playbooks(Request::new(ListPlaybooksRequest {}))
            .await
            .unwrap();

        // 7. Persistence
        service
            .list_persistence(Request::new(ListPersistenceRequest {
                implant_id: imp_id.to_string(),
            }))
            .await
            .unwrap();
